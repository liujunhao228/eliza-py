"""
数据库迁移脚本：为 sessions 表添加 end_reason 字段
...
async def migrate():
    ...
    async with aiosqlite.connect(db_path) as db:
        ...
        if "end_reason" in columns:
            print("✓ 字段 end_reason 已存在，无需迁移")
            return
        
        # 添加字段
        await db.execute("ALTER TABLE sessions ADD COLUMN end_reason VARCHAR(30)")
        await db.commit()
        
        print("✓ 迁移成功：已添加 end_reason 字段到 sessions 表")


async def rollback():
    """回滚迁移：删除 end_reason 字段"""
    db_path = settings.turing.database.url.replace("sqlite:///", "")
    ...
    async with aiosqlite.connect(db_path) as db:
        ...
        print("  CREATE TABLE sessions (...不含 end_reason...);")
...
async def verify():
    ...
    async with aiosqlite.connect(db_path) as db:
        ...
        if "end_reason" in columns:
            print("✓ 验证通过：end_reason 字段已存在")
            
            # 检查字段类型
            cursor = await db.execute(
                "SELECT name, type FROM PRAGMA_table_info('sessions') WHERE name='end_reason'"
            )
            col_info = await cursor.fetchone()
            if col_info:
                print(f"  字段信息：{col_info[0]} | 类型：{col_info[1]}")
        else:
            print("✗ 验证失败：end_reason 字段不存在")


async def main():
    if len(sys.argv) < 2:
        print("用法：python add_session_end_reason.py [migrate|rollback|verify]")
        sys.exit(1)
