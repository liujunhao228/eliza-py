...
async def get_match_service_dep():
    """获取匹配服务实例（依赖注入）"""
...
async def join_match_queue(
    ...
    match_service = Depends(get_match_service_dep),
):
    """
    加入匹配队列（概率分流模式）
    ...
    try:
        ...
        logger.warning(f"匹配请求参数错误：user_id={user_id}, error={e}")
        raise HTTPException(
