...
def test_unified_settings():
    ...
    print(f"  [OK] bots_dir: {settings.paths.bots_dir}")
...
def test_config_manager():
    ...
    enable_ltp = config_mgr.get('alice.enable_ltp')
    print(f"  [OK] get('alice.enable_ltp'): {enable_ltp}")

    print("\n[获取 Bot 配置]")
    bot_configs = config_mgr.list_bot_configs()
    print(f"  [OK] 已加载 Bot 配置：{bot_configs}")

    default_bot = config_mgr.get_bot_config("default")
    if default_bot:
        print(f"  [OK] 默认 Bot: {default_bot.name} ({default_bot.id})")
    else:
        print(f"  [WARN] 未找到默认 Bot 配置")

    print("\n[设置配置 (运行时)]")
    config_mgr.set('turing.server.port', 9000)
    new_port = config_mgr.get('turing.server.port')
    print(f"  [OK] set('turing.server.port', 9000) -> get(): {new_port}")

    # 重置
    config_mgr.reload()
    reset_port = config_mgr.get('turing.server.port')
...
def test_bot_loader():
    ...
    print("\n[加载 Bot 配置]")
    bot_configs = loader.load_all("bots")
    print(f"  [OK] 加载了 {len(bot_configs)} 个 Bot 配置")
